	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
				...
				<view class="content" :style='{"color":"#333","fontSize":"28rpx"}'>
					<rich-text style="font-size: 33upx;line-height: 50upx;letter-spacing: 5upx;" :nodes="detail.ziyuan"></rich-text>
				</view>
			</view>
		...
		methods: {
			...
			async init(){
				...
				this.detail = res.data;
																																												// 轮播图片
				this.swiperList = this.detail.peitu ? this.detail.peitu.split(",") : [];
																//修改富文本的图片样式
				this.detail.ziyuan = this.detail.ziyuan.replace(/img src/gi,"img style=\"width:100%;\" src");
																																			},
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discusskechengziyuan', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusskechengziyuan/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
