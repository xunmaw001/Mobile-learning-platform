<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
        ...
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.xingqi" type="text" placeholder="星期" ></input>
			</view>
			<view class="action">
	...
	<scroll-view scroll-x="true" class="tabView" :style='{"padding":"8rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,1)","backgroundColor":"#e2e2e2","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
        	<view class="tab" v-for="(item,index) in categoryList" :key="index" :class="categoryName===item.banjimingcheng?'tabActive':''" @tap="categoryClick(item.banjimingcheng)">{{item.banjimingcheng}}</view>
        </scroll-view>
...
<script>
	export default {
		...
		async onShow() {
			let res = await this.$api.list('banji', {page:1,limit:100});
			res.data.list.splice(0,0,{id:0,banjimingcheng:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.banji = '%' + this.categoryName + '%'
				}
			...
			async search(){
				...
				let searchForm = {
					...
					searchForm['banji'] = '%' + this.searchForm.banji + '%'
																...
																if(this.searchForm.xingqi){
					searchForm['xingqi'] = '%' + this.searchForm.xingqi + '%'
				}	
